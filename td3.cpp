...
void merge_telemetry(double **telemetries,
    ...
    for (int i = 0; i < tot_telemetries; i++)
        for (int j = 0; j < telemetries_sizes[i]; j++)
            global_telemetry = append_to_array(
                    telemetries[i][j],
                    global_telemetry,
    ...
    for (int i = 0; i < global_telemetry_current_size / 3 - 1; i++)
        for (int j = 0; j < global_telemetry_current_size / 3 - i - 1; j++)
            if (global_telemetry[3 * j] > global_telemetry[3 * (j + 1)]) {
                double temp[3] = {global_telemetry[3 * j], global_telemetry[3 * j + 1], global_telemetry[3 * j + 2]};
                global_telemetry[3 * j] = global_telemetry[3 * (j + 1)];
                global_telemetry[3 * j + 1] = global_telemetry[3 * (j + 1) + 1];
                global_telemetry[3 * j + 2] = global_telemetry[3 * (j + 1) + 2];
                global_telemetry[3 * (j + 1)] = temp[0];
                global_telemetry[3 * (j + 1) + 1] = temp[1];
                global_telemetry[3 * (j + 1) + 2] = temp[2];
            }
}
